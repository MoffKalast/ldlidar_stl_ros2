...
bool tryOpenPort(const std::string& port_name) {
	...
	// Set file status flags to blocking mode
	fcntl(fd, F_SETFL, 0);
	...
	return (n >= 0); // If read call didn't fail, port is available
}

void shutdownCleanup()
{
	if(system("sudo uhubctl -l 2 -a 0") != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Could not shutdown USB ports!");
    } else {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "USB ports successfully shut down.");
    }
}
...
int main(int argc, char **argv) {
	...
	while (!portReady && rclcpp::ok()) {
		portReady = tryOpenPort(port_name);
		if (!portReady) {
	...
	rclcpp::on_shutdown(shutdownCleanup);
